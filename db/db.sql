...
CREATE PROCEDURE SP_GETUNIDADES(IN IDPROPIN INT)
    BEGIN
        ...
        SELECT U.IdUnidad, U.Nombre AS NombreUnidad, (SELECT FN_GETNOMBREBYID(C.IdPersona)) AS Inquilino, (SELECT FN_GETPRECIORENTA(U.IdUnidad, TRUE)) AS Renta, DATE_FORMAT(C.FechaCreacion, '%d/%m/%Y') AS FechaInicio FROM UNIDAD AS U LEFT JOIN CONTRATO AS C ON U.IdUnidad = C.IdUnidad WHERE U.Propiedad = IDPROPIN AND U.Estatus = 'Activo';
    end;
