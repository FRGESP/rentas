...
RETURNS INT
    ...
    BEGIN
        ...
        SET SUMUNIDADES = (SELECT COUNT(U.IdUnidad) FROM UNIDAD AS U WHERE U.Propiedad = ID);
        RETURN SUMUNIDADES;
    end;
...
CREATE FUNCTION FN_GETUNIDADESOCUPADAS(IDPROP INT)
    ...
    BEGIN
        ...
        IF TIPOVAR = 'Edificio' THEN
            SET UNIDADESOCUPADAS = (SELECT COUNT(C.IdContrato) FROM CONTRATO AS C WHERE C.IdPropiedad = IDPROP) ;
            ELSE
...
CREATE PROCEDURE SP_GETPROPIEDADES()
    BEGIN
        SELECT P.IdPropiedad, P.Nombre AS NombrePropiedad, (SELECT FN_GETSUMUNIDADES(P.IdPropiedad)) AS NumUnidades, (SELECT FN_OBTENERDIRECCION(P.Direccion)) AS Direccion, P.Tipo, (SELECT FN_GETUNIDADESOCUPADAS(P.IdPropiedad)) AS Ocupados FROM PROPIEDAD AS P LEFT JOIN CONTRATO C on P.IdPropiedad = C.IdPropiedad LEFT JOIN PERSONA P2 on C.IdPersona = P2.IdPersona WHERE P.Estatus = 'Activo';
    end;
